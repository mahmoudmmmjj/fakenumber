...
# --- الإعدادات الأساسية (كودك الأصلي) ---
...
# --- إعدادات Firebase (الربط بملف google-services) ---
firebase_config = {
...
# --- إعدادات IVASMS (التوكن والكوكيز من ملفاتك) ---
IVAS_TOKEN = "mdWCez8pRLYEzgI4LPDcbCrMwJR96czg8PFWD7Sp"
IVAS_HEADERS = {
    'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36",
    'Cookie': "_fbp=fb.1.1776003227915.194465397404537711; cf_clearance=DYPKOA2ue315tKTGTtM_BydeHjQJKPmvaw9phITsOoI-1776629195-1.2.1.1-VmoaO1R37S_mE2vCKqwqcWvXbxDvAN8_a6ipyAB.0FJ90rzOna2WN.wF3_CWijy1sqUDGMYuoH_cUbcMEYX8WrNpF_bC2cM_g3xRSI1hg1FPMDBKcQtz45ozvFEHVQX766JkWTOYWApCobrqP4SjIdOJ5PZdZ8p423Pgxhoa9ErTdmrJOnuAco7a7WdrhD4zzX78qI9fsHhEjJ7_uyK2LxYE5xdJ23d05OAQnuwOYUapTLUE_z3TcCKyw30.hB.S3sX5HsjX3d1z3WVW28tkX2IZ4NhSsUNGwNrN_nM8GDX5sySrKHEmv_wtwH3hahjYjxgXRZ2YZOtiFY2K5RQqPg; XSRF-TOKEN=%3D; ivas_sms_session=%3D"
}

# --- إدارة البيانات (Firebase بديل JSON) ---
...
# --- وظيفة سحب الكود (Range Connection) ---
def fetch_sms(number, range_name):
    ...
    try:
        res = session.post(url, data=payload, headers=IVAS_HEADERS, timeout=8)
...
# --- الأوامر الأساسية (كاملة من كودك) ---
...
# --- Callbacks (كودك الأصلي) ---
@bot.callback_query_handler(func=lambda call: True)
def callbacks(call):
    n = db.child("nums").get().val() or {}; s = get_settings(); uid = str(call.from_user.id)
    if call.data == "u_get":
        ...
        if details and details.get('list'):
            ...
            bot.edit_message_text(f"◈ **الرقم:** `{num}`\n⏳ جاري السحب...", call.message.chat.id, call.message.message_id)
        ...
        u_count = len(db.child("users").get().val() or {})
        bot.answer_callback_query(call.id, f"📊 مستخدمين: {u_count}\n📦 خدمات: {len(n)}", show_alert=True)

# --- وظائف الإضافة والحظر (باقي الـ 270 سطر) ---
def cancel_markup():
...
def save_file_with_range(m, srv, cnt, rng):
    ...
    bot.send_message(m.chat.id, "✅ تم الإضافة لـ Firebase.", reply_markup=types.ReplyKeyboardRemove())
    admin_panel(m)
